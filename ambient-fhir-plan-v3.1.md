# Ambient Listening to FHIR Resources — Planning Document v3.1

**Status:** Revised planning draft (v3.1)
...
> **v3.1 patch (small fixes on top of v3):**
> - Corrected stray `answerCoding` wording → `valueCoding` (the R4 element name).
> - **Decision:** `questionnaireId` is the **stable FHIR logical id** for the demo (seeded via `PUT`), not a separate app-key — see §0.2.
> - Choice **save** payloads now carry `system` + `code` (not just the code) — §10, §8.
> - Conditional patient create now has explicit **0 / 1 / >1 match** fallback handling — §10.
> - `clinicalStatus` added to the transaction-Bundle allergy example — §7.10.
...
> - **Choice answers** now correctly map to `valueCoding` (not `answerString`).
...
```text
Frontend always sends the QUESTIONNAIRE LOGICAL ID.
...
**Demo decision:** `questionnaireId` is the **stable FHIR logical id** (not a separate app-key). Seed each questionnaire with a chosen, readable logical id via update-as-create (`PUT /Questionnaire/general-intake`) so the logical id is fixed and human-friendly (e.g. `general-intake`). This is why every example below uses `general-intake` as both the id and the slug — there is no second identifier to keep in sync.

This keeps a single source of truth (the stored `Questionnaire`) and means the saved response always references a resolvable, versioned canonical. Throughout the API in §10, `questionnaireId` = the **stable FHIR logical id**, and the canonical is derived server-side at save time.
...
3.  Nurse selects Questionnaire (by logical id).
...
```
Gotchas: `url` stable/canonical; bump `version` on change; `linkId`s are the contract across questionnaire, extraction, review UI, and response. **`choice` items define `answerOption.valueCoding`** — the response must echo the matched option as `answer.valueCoding` (see §7.3, §8).
...
**Choice mapping (important).** The LLM returns a plain string/label for a `choice` answer; the **backend** maps it to the matching `answerOption.valueCoding`. If no option matches, mark the item **needs-review** rather than inventing a coding. Never store a free `valueString` for a `choice` item. At save time the frontend sends the resolved choice as **`system` + `code`** (not the display string); the backend re-attaches `display` from the questionnaire's `answerOption` when building `answer.valueCoding`, so the stored coding stays consistent with the template.
...
**Intermediate JSON.** Note `questionnaireId` is the **stable FHIR logical id**; the backend derives the canonical (§0.2).
```json
{
...
All frontend calls go to the app backend (base path `/api`), never to IRIS. Throughout, `questionnaireId` = **stable FHIR logical id** (§0.2).
...
```
Notes: backend issues a **conditional create** — `POST /Patient` with header `If-None-Exist: identifier=http://example.org/fhir/mrn|MRN12345`. Per the FHIR conditional-create rules there are three outcomes the backend must handle:

- **0 matches** → IRIS creates the patient and returns `201 Created`. Backend responds `"created": true`.
- **1 match** → IRIS does **not** create; it returns the existing patient (`200 OK`). Backend responds `"created": false` and uses the returned id. (Repeat demo runs land here — no duplicates.)
- **>1 match** → IRIS returns `412 Precondition Failed` (the MRN isn't actually unique). Backend must **not** treat this as success: surface a `409`-style error to the frontend with `"matchType": "multiple"` so the nurse disambiguates manually, rather than silently picking one or creating a duplicate.

This mirrors the MRN-scan resolve/create/disambiguate logic in §6.3 — the only safe-by-default path when an identifier turns out non-unique is to stop and ask the nurse.
...
```json
{
  ...
  "answers": [
    ...
    { "linkId": "smoking-status", "valueType": "choice",
      "value": { "system": "http://snomed.info/sct", "code": "8517006" } }
  ],
  "acceptedSuggestions": [
...
7. **Identifier resolution (NEW)** → frontend sends logical id; backend derives versioned canonical at save time (§0.2). Don't let the frontend build canonicals.
